...
export const metadata = {
  title: "Offer a service — Outback Connections",
  description:
    "List your rural service so people can find you. Bore pumps, helicopter, drone, mechanic, sparky, plumber. Free.",
};

export const dynamic = "force-dynamic";

export default async function PostServiceOfferingPage() {
  const guard = await checkPostingGuard();
  if (!guard.ok && guard.reason === "not_signed_in") {
    redirect("/signin?next=/post/service/offering");
  }
  if (!guard.ok) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-10">
        <h1 className="text-2xl font-bold tracking-tight">Offer a service</h1>
